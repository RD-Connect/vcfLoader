    ...
    for chrom in configuration["chromosome"]:
        ...
        if (configuration["steps"]["transform"]):
            ...
            grouped.annotate_variants_expr([
                ...
                'va.samples = gs.filter(x=> x.dp >7 && x.gq> 19).map(g=>  {gq: g.gq, dp : g.dp, gt:intToGenotype(g.gt) , gtInt : g.gt, ad : ADsplit(g.ad.mkString(","),intToGenotype(gt)), sample : s}  ).collect()',
                'va.chrom=  v.contig',
                'va.pos = v.start',
                'va.ref= v.ref',
