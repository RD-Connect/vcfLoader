    ...
    for chrom in configuration["chromosome"]:
        ...
        if (configuration["steps"]["transform"]):
            ...
            grouped.annotate_variants_expr([
                ...
                'va.effs =  va.vep.transcript_consequences.map(x=>  {gene_name:  x.gene_symbol, effect_impact: x.impact ,transcript_id: x.transcript_id, effect : x.consequence_terms , gene_id : x.gene_id ,codon_change :x.hgvsc, amino_acid_change : x.hgvsp, exon_rank: x.exon, transcript_bioytpe: str(x.cds_start)+"/"+str(x.cds_end)})',
