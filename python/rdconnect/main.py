...
def main(hc,sqlContext):
    ...
    for chrom in configuration["chromosome"]:
        ...
        if (configuration["steps"]["annotateExomesGnomad"]):
            ...
            annotations.annotateVCF(hc,variants,utils.buildFileName(configuration["exomesGnomad_path"],chrom),destination+"/annotatedVEPdbnSFPCaddClinvarExGnomad/"+fileName,'va.gnomAD_Ex_AC = vds.info.gnomAD_Ex_AC[vds.aIndex-1], va.gnomAD_Ex_AF = vds.info.gnomAD_Ex_AF[vds.aIndex-1]')
        ...
        if (configuration["steps"]["annotateWGGnomad"]):
            ...
            annotations.annotateVCF(hc,variants,utils.buildFileName(configuration["genomesGnomad_path"],chrom),destination+"/annotatedVEPdbnSFPCaddClinvarExGnomadWGGnomad/"+fileName,'va.gnomAD_WG_AC = vds.info.gnomAD_WG_AC[vds.aIndex-1], va.gnomAD_WG_AF = vds.info.gnomAD_WG_AF[vds.aIndex-1]')
