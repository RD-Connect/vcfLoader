    ...
    for chrom in configuration["chromosome"]:
        ...
        if (configuration["steps"]["transform"]):
            ...
            grouped.annotate_variants_expr([
                ...
                'va.samples = gs.filter(x=> x.dp >7 && x.gq> 19).map(g=>  {gq: g.gq, dp : g.dp, gt:intToGenotype(g.gt) , gtInt : g.gt, ad : if(g.gt >0) g.ad[0]/g.ad.sum.toFloat else g.ad[1]/g.ad.sum.toFloat, sample : s}  ).collect()',
                'va.chrom=  v.contig',
                'va.pos = v.start',
                'va.ref= v.ref',
