...
def update_data_last_index(host, port, num_shards, num_replicas, user, pwd, data_project, data_index): 
	if not index.index_exists(host, port, "data_tracking", user, pwd):
		...
		data = """{"settings": { "index": { "number_of_shards": {}, "number_of_replicas": {} } }, "mappings":{ "1.0.0" :{ "properties": { "platform" : { "type" : "text", "index": "true" }, "index" : { "type" : "text" } } } } }""".format(num_shards, num_replicas)
