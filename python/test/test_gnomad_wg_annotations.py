...
class GnomADWGAnnotationsTests(BaseTestClass):
    ...
    def setUp(self):
        ...
        self.hc.import_vcf(self.config["gnomadWGVcfPath"]).split_multi().write(self.config["gnomadWGVdsPath"],overwrite=True)
        # Creating annotated variants with gnomAD wg
        annotations.annotateVCF(self.hc,variants,self.config["gnomadWGVdsPath"],self.config["sampleVdsPath"],'va.gnomAD_WG_AC = vds.info.gnomAD_WG_AC[vds.aIndex-1], va.gnomAD_WG_AF = vds.info.gnomAD_WG_AF[vds.aIndex-1]')
