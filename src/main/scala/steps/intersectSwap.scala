...
object intersectSwap {
  ...
  def apply(sc: org.apache.spark.SparkContext, rawSample: org.apache.spark.sql.DataFrame, chromList: String, destination: String, banda: (Int, Int), repartitions: Int) = {
    ...
    val bandsRDD :RDD[(Int,SwapData)]= bands.map(x => (x.getAs("pos").toString.toInt,SwapData(x.getAs("pos"), x.getAs("end_pos"), x.getAs("ref"), x.getAs("alt"), x.getAs("rs"), x.getAs("indel"), x.getAs("sampleId"), x.getAs("gq"), x.getAs("dp"), x.getAs("gt"), x.getAs("ad")))).rdd.repartitionAndSortWithinPartitions (binPartitioner)
    ...
    val variantsRDD :RDD[(Int,SwapDataThin)]= variants.map(x => (x.getAs("pos").toString.toInt,SwapDataThin(x.getAs("pos"), x.getAs("ref"), x.getAs("alt"), x.getAs("rs"), x.getAs("indel")))).rdd.repartitionAndSortWithinPartitions(binPartitioner)
