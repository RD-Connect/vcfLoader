...
object toEffectsGrouped{
  ...
  def main(sqlContext :org.apache.spark.sql.hive.HiveContext, umdAnnotated:org.apache.spark.sql.DataFrame,destination :String, chromList:String, banda:(Int,Int))= {
    ...
    val effGrouped = sqlContext.sql(
      ...
      """select pos,ref,alt,rs,indel, collect2( map('effect',effectsexploded.effect,'effect_impact',effectsexploded.effect_impact,'functional_class',effectsexploded.functional_class,'codon_change',effectsexploded.codon_change,'amino_acid_change',effectsexploded.amino_acid_change,
...
'exon_rank',effectsexploded.exon_rank,'geno_type_number',effectsexploded.geno_type_number,'UMD', IF(umd is NULL, '', umd))),
 array(collect2( map ('gp1_afr_af',populations.gp1_afr_af,'gp1_asn_af',populations.gp1_asn_af ,'gp1_eur_af',populations.gp1_eur_af,'gp1_af',populations.gp1_af,
    'esp6500_aa', populations.esp6500_aa, 'esp6500_ea', populations.esp6500_ea, 'exac', populations.exac))[0]),
 array(collect2(map('sift_pred', predictions.SIFT_pred, 'sift_score', predictions.SIFT_score, 'polyphen2_hvar_pred', predictions.polyphen2_hvar_pred,
